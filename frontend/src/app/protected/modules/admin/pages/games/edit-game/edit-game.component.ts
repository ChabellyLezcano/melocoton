    ...
    this.gameService.updateGame(this.gameId, this.editGameForm.value).subscribe(
      (response: GameResponse) => {
        if (response.ok) {
          ...
          this.router.navigate(['/dashboard/game', this.gameId]);
        } else {
          Swal.fire(
            'Error',
            response.msg || 'Ha ocurrido un error al actualizar el juego',
            'error'
          );
        }
      },
